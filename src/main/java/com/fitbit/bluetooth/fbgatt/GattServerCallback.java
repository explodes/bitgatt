...
class GattServerCallback extends BluetoothGattServerCallback {
    ...
    GattServerCallback() {
        ...
        Looper looper = fitbitGatt.getFitbitGattAsyncOperationThread().getLooper();
    ...
    public void onConnectionStateChange(BluetoothDevice device, int status, int newState) {
        ...
        handler.post(() -> {
            for (GattServerListener listener : copy) {
                listener.onServerConnectionStateChange(device, status, newState);
            }
            GattServerConnection conn = fitbitGatt.getServer();
            GattConnection gattConnection = fitbitGatt.getConnection(device);
                ...
                switch (newState) {
                    ...
                    case BluetoothProfile.STATE_DISCONNECTED:
                        for (ServerConnectionEventListener asyncListener : conn.getConnectionEventListeners()) {
                            ...
                            asyncListener.onServerConnectionStateChanged(device, result, conn);
                        }

                        break;
                    case BluetoothProfile.STATE_CONNECTED:
                        if (gattConnection == null) {
                            fitbitGatt.addConnectedDevice(device);
                        }
                        for (ServerConnectionEventListener asyncListener : conn.getConnectionEventListeners()) {
                            // since this is async, the result status is irrelevant so it will always be
                            // success because we received this data
    ...
    public void onServiceAdded(int status, BluetoothGattService service) {
        ...
        handler.post(() -> {
            for (GattServerListener listener : copy) {
                listener.onServerServiceAdded(status, service);
            }
        });
    }
    ...
    public void onCharacteristicReadRequest(BluetoothDevice device, int requestId, int offset, BluetoothGattCharacteristic characteristic) {
        ...
        handler.post(() -> {
            ...
            for (GattServerListener listener : copy) {
                listener.onServerCharacteristicReadRequest(device, requestId, offset, copyOfCharacteristic);
            }
            if (conn == null) {
                Timber.v("[%s] Gatt was null, we could be mocking, if so we can't notify async", getDeviceMacFromDevice(device));
            } else {
                ...
                if (conn.getConnectionEventListeners().isEmpty() && copy.isEmpty()) {
                    ...
                    for (ServerConnectionEventListener asyncListener : conn.getConnectionEventListeners()) {
                        ...
                        TransactionResult result = new TransactionResult.Builder()
                                ...
                                .resultStatus(TransactionResult.TransactionResultStatus.SUCCESS).build();
                        asyncListener.onServerCharacteristicReadRequest(device, result, conn);
                    }
                }
            }
        });
    ...
    public void onCharacteristicWriteRequest(BluetoothDevice device, int requestId, BluetoothGattCharacteristic characteristic, boolean preparedWrite, boolean responseNeeded, int offset, byte[] value) {
        ...
        handler.post(() -> {
            BluetoothGattCharacteristicCopy copyOfCharacteristic = new GattUtils().copyCharacteristic(characteristic);
            for (GattServerListener listener : copy) {
                listener.onServerCharacteristicWriteRequest(device, requestId, copyOfCharacteristic, preparedWrite, responseNeeded, offset, value);
            }
                ...
                if (conn.getConnectionEventListeners().isEmpty() && copy.isEmpty()) {
                    ...
                    for (ServerConnectionEventListener asyncListener : conn.getConnectionEventListeners()) {
                        ...
                        TransactionResult result = new TransactionResult.Builder()
                                ...
                                .resultStatus(TransactionResult.TransactionResultStatus.SUCCESS).build();
                        asyncListener.onServerCharacteristicWriteRequest(device, result, conn);
                    }
                }
            }
        });
    ...
    public void onDescriptorReadRequest(BluetoothDevice device, int requestId, int offset, BluetoothGattDescriptor descriptor) {
        ...
        handler.post(() -> {
            ...
            for (GattServerListener listener : copy) {
                listener.onServerDescriptorReadRequest(device, requestId, offset, copyOfDescriptor);
            }
            if (conn == null) {
                Timber.v("[%s] Gatt was null, we could be mocking, if so we can't notify async", getDeviceMacFromDevice(device));
            } else {
                if (conn.getConnectionEventListeners().isEmpty() && copy.isEmpty()) {
                    ...
                    for (ServerConnectionEventListener asyncListener : conn.getConnectionEventListeners()) {
                        ...
                        TransactionResult result = new TransactionResult.Builder()
                                ...
                                .offset(offset)
                                .resultStatus(TransactionResult.TransactionResultStatus.SUCCESS).build();
                        asyncListener.onServerDescriptorReadRequest(device, result, conn);
                    }
                }
            }
        });
    ...
    public void onDescriptorWriteRequest(BluetoothDevice device, int requestId, BluetoothGattDescriptor descriptor, boolean preparedWrite, boolean responseNeeded, int offset, byte[] value) {
        ...
        handler.post(() -> {
            BluetoothGattDescriptorCopy copyOfDescriptor = new GattUtils().copyDescriptor(descriptor);
            for (GattServerListener listener : copy) {
                listener.onServerDescriptorWriteRequest(device, requestId, copyOfDescriptor, preparedWrite, responseNeeded, offset, value);
            }
            if (conn == null) {
                Timber.v("[%s] Gatt was null, we could be mocking, if so we can't notify async", getDeviceMacFromDevice(device));
            } else {
                if (conn.getConnectionEventListeners().isEmpty() && copy.isEmpty()) {
                    ...
                    for (ServerConnectionEventListener asyncListener : conn.getConnectionEventListeners()) {
                        ...
                        TransactionResult result = new TransactionResult.Builder()
                                ...
                                .responseRequired(responseNeeded)
                                .resultStatus(TransactionResult.TransactionResultStatus.SUCCESS).build();
                        asyncListener.onServerDescriptorWriteRequest(device, result, conn);
                    }
                }
            }
        });
    ...
    public void onExecuteWrite(BluetoothDevice device, int requestId, boolean execute) {
        ...
        handler.post(() -> {
            for (GattServerListener listener : copy) {
                listener.onServerExecuteWrite(device, requestId, execute);
            }
        });
    }
    ...
    public void onNotificationSent(BluetoothDevice device, int status) {
        ...
        handler.post(() -> {
            for (GattServerListener listener : copy) {
                listener.onServerNotificationSent(device, GattStatus.getStatusForCode(status).ordinal());
            }
        });
    }
    ...
    public void onMtuChanged(BluetoothDevice device, int mtu) {
        ...
        handler.post(() -> {
            for (GattServerListener listener : copy) {
                listener.onServerMtuChanged(device, mtu);
            }
            GattServerConnection conn = fitbitGatt.getServer();
            if (conn == null) {
                Timber.v("[%s] Gatt was null, we could be mocking, if so we can't notify async", getDeviceMacFromDevice(device));
                ...
                for (ServerConnectionEventListener asyncListener : conn.getConnectionEventListeners()) {
                    ...
                    TransactionResult result = new TransactionResult.Builder()
                            ...
                            .mtu(mtu)
                            .resultStatus(TransactionResult.TransactionResultStatus.SUCCESS).build();
                    asyncListener.onServerMtuChanged(device, result, conn);
                }
            }
        });
    }
    ...
    public void onPhyUpdate(BluetoothDevice device, int txPhy, int rxPhy, int status) {
        ...
        handler.post(() -> {
            for (GattServerListener listener : copy) {
                listener.onServerPhyUpdate(device, txPhy, rxPhy, status);
            }
        });
    }
    ...
    public void onPhyRead(BluetoothDevice device, int txPhy, int rxPhy, int status) {
        ...
        handler.post(() -> {
            for (GattServerListener listener : copy) {
                listener.onServerPhyRead(device, txPhy, rxPhy, status);
            }
        });
    }
